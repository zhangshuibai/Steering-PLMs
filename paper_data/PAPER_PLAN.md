...
│
├── analysis/                            分析脚本 (reproducibility)
│   ├── README.md                        脚本运行顺序 + 依赖说明
│   ├── 01_compute_proxies.py
│   ├── 02_compute_ppl_3b.py
│   ├── 03_goldilocks_analysis.py
│   ├── 04_filter_chisq.py
│   ├── 05_filter_task_optimal.py        ← Main Table 1 生成
│   ├── 06_filter_percentile.py
│   ├── 07_rejection_sampling.py
│   ├── 08_random_aa_baseline.py
│   ├── 09_proxy_u_sweep.py
│   ├── 10_filter_within_setting.py
│   └── make_master_csv.py               聚合 proxy 到 master.csv
│
├── figures/                             paper 图(待填)
│
├── tables/                              paper 表(待填)
...
├── sol_easy/ (~5.6 MB)                  ┐
├── sol_hard/ (~3.4 MB)                  │
├── therm_easy/ (~3.5 MB)                │── 6 个 task,结构相同:
├── therm_hard/ (~3.9 MB)                │   ├── master.csv (NEW! 合并 view,分析入口)
├── trpb/ (~2.3 MB)                      │   ├── generated/ (5 settings × seqs)
└── gfp/ (~1.5 MB)                       ┘   ├── oracle/ (5 scored CSVs)
                                             ├── esmfold/ (merged per-seq pLDDT)
                                             └── proxy/ (per-seq Mahal+GLP resid+ppl)
```

### 推荐分析入口: `master.csv`

每 task 现在都有一个 `master.csv`,merge 了所有 per-seq 指标:

```python
import pandas as pd
df = pd.read_csv("paper_data/sol_easy/master.csv")
# Columns: task, setting, sequence, plddt, oracle, mahal, glp_resid, ppl_650m, ppl_3b
# 2500 rows (5 settings × 500 seqs,fitness tasks 是 1000 rows)
df.head()
```

**这是做所有 paper 分析的推荐入口**。其他子目录(generated/oracle/esmfold/proxy)是原始数据备份。
...
| 日期 | 版本 | 主要变化 |
|---|---|---|
| 2026-04-19 | v0.1 | 初版,双 contribution(Modifier + Filter)架构 |
| 2026-04-20 | v0.2 | **聚焦**: 删除 Modifier 作为主 claim,仅保留 Filter。Modifier 移至 §6 Limitations 作 negative result |
| 2026-04-20 | v0.3 | **Main Table 1 调整**: 从统一 allL α=2 改为 task-optimal setting(sol: allL α=2, therm: L17 α=1),让 4 splits 都双赢 |
| 2026-04-21 | v0.4 | **paper_data 整理**: 加 `analysis/`(10 个分析脚本 + README)、`figures/`、`tables/` 空目录、每 task 的 `master.csv`(分析入口) |
