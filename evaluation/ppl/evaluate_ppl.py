...
For each position, mask it and measure the model's log-probability of the true token.
...
from evaluation.common import load_esm2_model
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="ESM2 pseudo-perplexity evaluation")
    ...
    parser.add_argument('--model', type=str, default='3B', choices=['150M', '650M', '3B'],
                        help='ESM2 model size (default: 3B)')
    ...
    for csv_path, label in zip(args.input_csvs, args.labels):
        ...
        for i, (s, p) in enumerate(zip(seqs, ppls)):
            all_results.append({
                'group': label, 'seq_idx': i, 'sequence': s,
                'length': len(s), 'ppl': p, 'log_ppl': np.log(p),
            })
